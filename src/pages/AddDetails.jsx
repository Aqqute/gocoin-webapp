...
const AddDetails = () => {
  ...
  return (
    <div
      className={`min-h-screen w-full flex items-center justify-center px-2 py-8 ${isDark ? "bg-gradient-to-br from-[#0f2027] via-[#2c5364] to-[#232526] text-white" : "bg-gradient-to-br from-[#f8ffae] via-[#43cea2] to-[#185a9d] text-black"}`}
      style={{ minHeight: '100dvh' }}
    >
      <div
        className="w-full max-w-xl mx-auto rounded-2xl shadow-xl p-6 md:p-10 flex flex-col gap-6 md:gap-8 backdrop-blur-md bg-white/70 dark:bg-[#232526]/80 border border-white/20"
        style={{
          boxShadow: isDark
      ...
      >
        <h2 className="text-2xl md:text-3xl font-extrabold text-center tracking-tight flex items-center justify-center gap-2">
          <User className="inline-block text-orange-500" size={28} />
          Complete Your Profile
        </h2>
        {/* Country */}
        <form onSubmit={handleCountrySubmit} className="flex flex-col md:flex-row items-center gap-3 md:gap-4 mb-1">
          <label className="flex items-center gap-2 font-semibold text-base md:text-lg whitespace-nowrap">
            <Globe className="text-blue-500" size={20} /> Country
          </label>
          <input
            type="text"
            className="flex-1 px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-700 shadow-inner bg-white/90 dark:bg-[#232526]/80 text-base focus:outline-none focus:ring-2 focus:ring-orange-500"
            value={country}
            onChange={e => setCountry(e.target.value)}
            placeholder="e.g. Nigeria"
            required
          />
          <button type="submit" className="flex items-center gap-2 bg-gradient-to-r from-orange-400 to-pink-500 text-white px-5 py-2 rounded-lg font-bold shadow-md hover:scale-105 transition-transform" disabled={loading}>
            <Send size={16} /> {loading ? "Saving..." : "Save"}
          </button>
        </form>
        {/* State/Region */}
        <form onSubmit={handleStateSubmit} className="flex flex-col md:flex-row items-center gap-3 md:gap-4 mb-1">
          <label className="flex items-center gap-2 font-semibold text-base md:text-lg whitespace-nowrap">
            <Globe className="text-blue-500" size={20} /> State/Region
          </label>
          <input
            type="text"
            className="flex-1 px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-700 shadow-inner bg-white/90 dark:bg-[#232526]/80 text-base focus:outline-none focus:ring-2 focus:ring-orange-500"
        ...
        {/* Phone Number */}
        <form onSubmit={handlePhoneSubmit} className="flex flex-col md:flex-row items-center gap-3 md:gap-4 mb-1">
          <label className="flex items-center gap-2 font-semibold text-base md:text-lg whitespace-nowrap">
            <Phone className="text-green-500" size={20} /> Phone
          </label>
          <input
            type="tel"
            className="flex-1 px-4 py-2 rounded-lg border border-gray-200 dark:border-gray-700 shadow-inner bg-white/90 dark:bg-[#232526]/80 text-base focus:outline-none focus:ring-2 focus:ring-orange-500"
        ...
        <div className="flex flex-col gap-2 md:gap-3">
          <label className="flex items-center gap-2 font-semibold text-base md:text-lg">
            <span className="bg-gradient-to-r from-blue-400 to-purple-500 bg-clip-text text-transparent">Socials</span>
          </label>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2 md:gap-3">
            <div className="flex items-center gap-2 bg-white/80 dark:bg-[#232526]/70 rounded-lg px-3 py-2 border border-gray-100 dark:border-gray-700">
              <TelegramIcon />
            ...
            <div className="flex items-center gap-2 bg-white/80 dark:bg-[#232526]/70 rounded-lg px-3 py-2 border border-gray-100 dark:border-gray-700">
              <Facebook className="text-blue-700" size={18} />
        ...
        {/* Show current values */}
        {user && (
          <div className="mt-5 text-xs md:text-sm text-center bg-white/60 dark:bg-[#232526]/60 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
